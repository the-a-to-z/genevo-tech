...
class Mod_globals extends CI_Model {
    ...
    public function delete_where_not_in($table, $arr_where_not_in = array(),$arr_where = array()) {
        if (!is_array($arr_where_not_in) || count($arr_where_not_in) <= 0)
            return FALSE;
        foreach ($arr_where_not_in as $field => $value) {
            $this->db->where_not_in($field, $value);
        }
        foreach ($arr_where as $field_where => $value_where) {
