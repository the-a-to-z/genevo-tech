...
if(!function_exists('getMenu')){
	 function getMenu($level){
		 $CI = &get_instance();
		 $CI->load->model('mod_pages');
		 $data = $CI->mod_pages->getMenu($level);
		 return $data;
	 }
}
if(!function_exists('getDataPostion')){
	function getDataPostion($postionId,$type,$menu_name){
		$CI = &get_instance();
		$CI->load->model('mod_pages');
		$data = $CI->mod_pages->getDataPostion($postionId,$type,$menu_name);
		return $data;
	}
}
if(!function_exists('getDataContact')){
	function getDataContact(){
		$CI = &get_instance();
		$CI->load->model('mod_pages');
		$data = $CI->mod_pages->getDataSiting();
		return $data;
	}
}
if(!function_exists('getSlideShow')){
	function getSlideShow($url){
		$CI = &get_instance();
		$CI->load->model('mod_pages');
		$data = $CI->mod_pages->getSlider($url);
		return $data;
	}
}
...
if(!function_exists('getProduct')){
	function getProduct($search){
		$CI = &get_instance();
		$CI->load->model('mod_pages');
		$data = $CI->mod_pages->getProduct($search);
		return $data;
	}
}

if(!function_exists('checExitkSlideShow')){
	function checExitkSlideShow($menu_id,$sli_id){
		$CI = &get_instance();
		$CI->load->model('mod_globals');
		$data = $CI->mod_globals->check_data_exist("tbl_sliders_menu",array('sli_id' => $sli_id,'men_id'=>$menu_id) );
		return $data;
	}
}
